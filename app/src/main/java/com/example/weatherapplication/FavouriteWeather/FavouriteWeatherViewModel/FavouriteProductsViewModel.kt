...
class FavouriteProductsViewModel(private val productsRepositoryInterface_Instance_ConstructorParameter_InFavouriteWeatherViewModel: WeatherRepositoryInterface): ViewModel() {
                //_product
    ...
    fun deleteProduct_InFavouriteProductsViewModel(product:Products){
        viewModelScope.launch(Dispatchers.IO){
            productsRepositoryInterface_Instance_ConstructorParameter_InFavouriteWeatherViewModel.deleteProduct_FromLDS_InProductsRepository(product)
            getAllLocalProducts_StoredInDatabase_InFavouriteProductsViewModel()
        }
    }
    ...
    fun getAllLocalProducts_StoredInDatabase_InFavouriteProductsViewModel(){
        viewModelScope.launch(Dispatchers.IO){
            productsRepositoryInterface_Instance_ConstructorParameter_InFavouriteWeatherViewModel.getAllStoredProducts_FromLDS_InProductsRepository().collect{ products -> productsMutableLiveDataList_InFavouriteProductsViewModel.postValue(products)}
        }
    }
